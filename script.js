...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelectorAll(".card, .partner-card, .product-card, .testimonial-card")
    .forEach((card) => {
  ...
  const lbClose = lightboxModal
    ? lightboxModal.querySelector(".lb-close")
    : null;
  ...
  document.querySelectorAll(".lightbox-trigger").forEach((trigger) => {
    trigger.addEventListener("click", (e) => {
      e.preventDefault();
      const large =
        trigger.getAttribute("data-large") || trigger.querySelector("img")?.src;
      const caption =
    ...
    contactForm.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        if (successFlag) {
          ...
          const redirectInput = contactForm.querySelector(
            'input[name="redirect"]'
          );
  ...
  // Toggle Home nav visibility: show only on products, contact, or careers pages
  (function () {
    function findHomeLink() {
      ...
      let el = document.querySelector(selectors.join(","));
      if (!el) {
        // fallback: find link with Home text
        el = Array.from(
          document.querySelectorAll(".navbar-nav a.nav-link")
        ).find((a) => /\bhome\b/i.test(a.textContent));
      }
      return el;
    }
    ...
    function updateHomeVisibility() {
      ...
      // Hide Home when on the homepage (root or index.html). Show on all other pages.
      const isHome = filename === "" || filename === "index.html";
      homeLink.style.display = isHome ? "none" : "";
    }

    // Run once on load and also when history changes (back/forward)
    updateHomeVisibility();
    window.addEventListener("popstate", updateHomeVisibility);
  })();

  // Measure navbar and apply a body class when it overlaps the section heading
  (function applyNavbarOffset() {
    ...
    function recompute() {
      ...
      const navHeight = Math.round(navbar.offsetHeight || navRect.height || 72);
      document.documentElement.style.setProperty(
        "--nav-height",
        navHeight + "px"
      );

      // Find the first visible section title on the page
      const title = document.querySelector(".section-title");
      if (!title) {
        // still set a default but remove offset class
        document.body.classList.remove("has-nav-offset");
        return;
      }

      const titleRect = title.getBoundingClientRect();

      // If navbar bottom overlaps the title top (or sits at the very top), enable offset
      const overlap = navRect.bottom > titleRect.top + 1 || navRect.top <= 0;

      if (overlap) {
        document.body.classList.add("has-nav-offset");
      } else {
        document.body.classList.remove("has-nav-offset");
      }
    }

    // Run once after load to catch initial layout
    window.addEventListener("load", recompute);
    // Also run on DOM ready and resize/scroll (navbar can change on scroll)
    recompute();
...
document.addEventListener("DOMContentLoaded", function () {
  try {
    ...
    document.querySelectorAll(".navbar .nav-link").forEach((link) => {
      ...
      if (
        hrefFile === path ||
        (hrefFile === "index.html" && (path === "" || path === "index.html"))
      ) {
